...
# HK Comment] 비밀번호는 *로 표시, '취소','입력' 버튼 추가
# 인터페이스 이식성 고려할 것
# 미세먼지 관련 지수 표시하는 인터페이스 '이해성(Understandability)' 고려할 것
# 장비를 가동하는 것은 더욱 신중하게.
# 장비를 인공지능 모드에 의해서 구동시 '구동 시키겠습니까?' 추천
# 실시간 업데이트에서 단순히 조회만 하고 싶다면 조회 + 인공지능 모드의 장비에어 기능을 분리하여 처리하는 것을 추천
# 시뮬레이션 기능 추가 할 것
# 실시간 업데이트 또는 인공지능 모드에서 장비를 제어 할 수 있게 관련 상수를 변수 처리함
# 더욱 완벽하게 하려면 해당 변수를 환경설정 할 수 있게 한다.
# 공기청정기를 가동시키지 않는 조건에 대한 테스트를 해볼것
# 검색하고 싶은 영화입력할 때 번호로 처리
