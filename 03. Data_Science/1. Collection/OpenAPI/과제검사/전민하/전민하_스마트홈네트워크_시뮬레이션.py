...
import threading
import ctypes
# HK Comment] 스피커도 시뮬레이션 할 수 있도록
# 버전 넘버
# 장비에 스피커 추가
# 스피커로 출력 되는 메세지는 별도로 표시할 것
# 예) 스피커 안내] "현재 온도는 -1입니다"
# 미세먼지 안내시 "현재 미세먼지 농도가 수치 (좋음) 입니다."
# 미세먼지 수치를 상수에서 변수처리 할 것. 데모시 즉시 변경하여 데모가능하도록
g_AI_speaker = False
g_Radiator = False
g_Gas_Valve = False
...
def get_simulation_humidity(filename):
    ...
    for i in result:
        if 'REH' == i['category']:
            humidity_list.append(i)

    if int(humidity_list[0]["fcstValue"]) < 40:
        print("건조 합니다.")
        if g_humidifier == False:
            ...
            print("가습기가 켜져 있습니다.")
    elif int(humidity_list[0]['fcstValue']) > 60:
        print("습도가 높습니다.")
    ...
    while True:
        if g_AI_Mode == True:
            if time.strftime("%M%S") == '4530':
                ...
                print("메뉴를 선택하세요: ")
                time.sleep(1)
            elif time.strftime("%H%M%S") == '1405300':
                weather_main()
                ...
                if rain_list[0]["fcstValue"] != 0:
                    ...
                    print_main_menu()
                    print("메뉴를 선택하세요: ")
                print("현재 온도는 %d입니다." %(temperature_list[0]["fcstValue"]))
                time.sleep(1)
        else:
            continue
