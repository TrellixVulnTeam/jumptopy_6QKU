...
# 실시간 업데이트를 하여 RN1 값이 있고 발코니 창이 열려 있는 경우 발코니 창을 닫도록 구현
...
def getForecastTimeDataResponse(base_date,base_time,nx,ny):
    ...
    parameters += "&nx=" + nx
    parameters += "&ny=" + ny
    parameters += "&_type=json&serviceKey=" + access_key +"&pageNo=2"
...
def get_weather_info():
    try:
        ...
        base_time = time.strftime("%H%M", time.localtime(time.time()))
        base_Time = int(base_time)-100
        with open("%s_%s_Weather.json" %(base_date,str(base_Time)), encoding='UTF8') as json_file:
            json_object = json.load(json_file)
            json_string = json.dumps(json_object)
            retJson = json.loads(json_string)
            for retJson in retJson:
    ...
    for retJson in retJson:
        if(retJson['category']) == 'REH':
            ...
            if retJson['fcstValue'] > 80:
                ...
                print("습도가 %s%% 입니다. 제습기를 가동할 필요가 없습니다."%str(retJson['fcstValue']))
                break
...
def Dry_forecast_simulation():
    ...
    for retJson in retJson:
        if(retJson['category']) == 'REH':
            print("현재습도 = %s%%" %str(retJson['fcstValue']))
            if retJson['fcstValue'] < 40:
                if g_humidifier == False:
                    print("\n***습도가 낮으므로(현재%s%%) 가습기를 가동하겠습니다.***"%str(retJson['fcstValue']))
...
def Sunny_Day_simulation():
    ...
    for retJson in retJson:
        if(retJson['category']) == 'SKY':
            print("현재 구름양 = %s%% "%str(retJson['fcstValue']))
            if retJson['fcstValue'] <= 2:
                if g_humidifier == False and g_dehumidifier == True:
                    ...
                    print("***화창한 날씨입니다. 현재 구름량은 %s%% 입니다."%str(retJson['fcstValue']))
                    break
