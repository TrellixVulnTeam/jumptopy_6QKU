...
def check_device_status():
    #HK Comment] 라인 수 줄일 것 (1시간 이것 만 고민해 볼것, 3~4줄로 가능)
    print("\n난방기 상태: ",end='')
    if(g_Radiator == True): print("작동")
    elif(g_Radiator == False): print("정지")

    print("가스밸브 상태: ",end='')
    if(g_Gas_Valve == True): print("Open")
    elif(g_Gas_Valve == False): print("Cloase")
    ...
    print("제습기 상태: ",end='')
    if(g_humidity == True): print("작동")
    elif(g_humidity == False): print("정지")

    print("가습기 상태: ",end='')
    if(g_humidifier == True): print("작동")
    elif(g_humidifier == False): print("정지")
...
입력하세요: """))
    if simulation_number == 1:
        ...
        global g_humidity
        global g_Max_Acceptable_humidity
        global g_Min_Acceptable_humidity
        real_weather_list = []
        real_weather = {"baseDate": 20180131,
                        "baseTime": 1430,
                        "category": "REH",
        ...
        elif humidity_AI_system < g_Min_Acceptable_humidity_level and g_humidity == False:
            print("습도 :", humidity_AI_system, "%", "입니다. 가습기를 작동해주세요.")
        with open('weather.json', 'w', encoding='utf8') as outfile:
            readable_result = json.dumps(real_weather_list, indent=4, sort_keys=True, ensure_ascii=False)
            outfile.write(readable_result)
        ...
        if humidifier_AI_system > g_Max_Acceptable_humidity_level and g_humidifier == True:
            ...
            print("습도 :", humidifier_AI_system, "%", "입니다. 제습기를 작동해주세요.")
        with open('weather.json', 'w', encoding='utf8') as outfile:
            readable_result = json.dumps(real_weather_list, indent=4, sort_keys=True, ensure_ascii=False)
            outfile.write(readable_result)
