    ...
    if(retJson[0]['category']) == 'REH':
        ...
        if retJson[0]['fcstValue'] > 80:
            if g_dehumidifier == False:
                ...
                print("\n***습도가 높으므로 열려있는 창문을 닫고, 제습기를 가동하겠습니다.***")
                g_Balcony_Windows = not g_Balcony_Windows
                g_dehumidifier = not g_dehumidifier
                check_device_status()
...
def Dry_forecast_simulation():
    ...
    if(retJson[0]['category']) == 'REH1':
        ...
        if retJson[0]['fcstValue'] < 40:
            if g_Balcony_Windows and g_humidifier == False:
                print("\n***습도가 낮으므로 닫혀 있는 창문을 열고, 가습기를 가동하겠습니다.***")
                g_Balcony_Windows = not g_Balcony_Windows
                ...
                print("***습도가 낮으므로 가습기가 작동중입니다. 습도를 높이기 위해 창문을 열겠습니다.***")
                g_Balcony_Windows = not g_Balcony_Windows
            else:
...
def Sunny_Day_simulation():
    ...
    if(retJson[0]['category']) == 'SKY':
        ...
        if retJson[0]['fcstValue'] <= 2:
            if g_humidifier and g_dehumidifier and g_Balcony_Windows == False:
                print("\n***화창한 날씨이므로 창문을 열겠습니다.***")
                g_Balcony_Windows = not g_Balcony_Windows
                check_device_status()
            elif g_humidifier and g_dehumidifier == True and g_Balcony_Windows == False:
                print("\n***화창한 날씨이므로 가습기/제습기의 작동을 정지하고 창문을 열겠습니다.***")
                g_dehumidifier = not g_dehumidifier
                g_humidifier = not g_humidifier
                g_Balcony_Windows = not g_Balcony_Windows
                check_device_status()
                ...
                g_dehumidifier = not g_dehumidifier
            # else:
                # print("***화창한 날씨입니다. 밀린 빨래를 하면 어떨까요?***")
