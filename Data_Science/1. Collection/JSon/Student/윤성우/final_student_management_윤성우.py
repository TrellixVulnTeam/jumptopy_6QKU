...
def info_show_indiviual(result):
    ...
    for i in range(len(result)):
        ...
        print("주소 (예 : 한양시 종로구 우리집) : %s\n" % result[i]['address'])
        print("<< 수강정보 >>")
        print("-- 과거 수강 횟수 (예 : 1) : %s " % result[i]['past_record'])
        try:
            for j in range(len(result[i].get('recent_info'))):
                print("-- 현재 학생의 수강 정보 < %d >" % (j+1))
                print("   강의코드 (예 : IB171106) : %s"  % result[i].get('recent_info')[j]['강의코드'])
                print("   강의명   (예 : IoT 강의) : %s"  % result[i].get('recent_info')[j]['강의명'])
                print("   강사명     (예 : 문재인)   : %s"  % result[i].get('recent_info')[j]['강사명'])
                print("   개강일   (예 : 2017-11-06) : %s"  % result[i].get('recent_info')[j]['개강일'])
                print("   종료일   (예 : 2018-09-05) : %s"  % result[i].get('recent_info')[j]['종료일'])
        except : pass
        print("=================  회원 정보 조회 성공  ====================")
        print("")
...
def info_start(path):
    try:
        with open(path + '\ITT_Student.json', encoding='UTF8') as json_file:
        ...
        with open(path + '\ITT_Student.json', 'w', encoding='UTF8') as outfile:
